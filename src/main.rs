...
async fn execute_sell(
    ...
    let tx_approve = TransactionRequest::new()
        .to(token_in)
        .data(approve_calldata.0)
        .gas(100_000)
        .gas_price(client.provider().get_gas_price().await?);
...
async fn main() -> anyhow::Result<()> {
    ...
    let wallet = if !config.private_key.is_empty() {
        config
            .private_key
            .parse::<LocalWallet>()?
            .with_chain_id(chain_id) // 这是为了防止重放攻击（Replay Attack), 以太坊交易签名标准（EIP-155）要求签名中包含链 ID，这样在 Base 链上签名的交易就不能被恶意拿到以太坊主网或其他链上去广播执行
    } else {
        if !config.shadow_mode {
